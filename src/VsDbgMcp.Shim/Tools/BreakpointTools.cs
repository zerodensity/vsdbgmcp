...
namespace VsDbgMcp.Shim.Tools
{
    ...
    public sealed class BreakpointTools : ToolBase
    {
        ...
        public Task<string> BpSet(
            ...
            [Description("Keep this tracepoint's records in a buffer of their own, read back with trace_read. Without it the records go to the Debug pane, mixed in with everything else the program logs, with no timestamps and no hit numbers. The message is written between markers of this server's own, which are literal text: a record arriving proves the line was reached even when none of its expressions evaluated.")] bool collect = false,
        ...
        [Description("List every breakpoint with its bind state and hit count. A bound breakpoint that breaks carries a count, totalled over every place it bound, so hits 0 means it was never reached rather than that nothing was counted; the count can lag the stop you are sitting on by one. A tracepoint carries none, because only hits that broke are counted - read trace_read instead. Unbound ones are marked and carry the reason they did not bind.")]
        ...
        [Description("Read what a collecting tracepoint has logged: only that breakpoint's records, each with the time it arrived and which hit it was. Set one with bp_set(logMessage: ..., collect: true). This is how to answer 'how often does this run, and in what order against that', without inferring it from how records interleave in the Debug pane. The reply says how many records were collected in total, so a tail that misses some is visible rather than silent. An empty stream is not evidence the line never ran: it is equally what records that never arrived look like, and the reply says which of the two it can rule out and what would settle it.")]
